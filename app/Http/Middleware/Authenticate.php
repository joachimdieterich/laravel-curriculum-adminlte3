...
class Authenticate extends Middleware
{
    ...
    // TODO: needs testing
    // verifies the token from the OpenID Connect ID Token
    protected function verifiyToken($idtoken)
    {
        $config = \Lcobucci\JWT\Configuration::forSymmetricSigner(
            new \Lcobucci\JWT\Signer\Rsa\Sha256(),
            \Lcobucci\JWT\Signer\Key\InMemory::file(base_path('oauth-public.key')) // This is the public key generated by passport. You need to share it.
        );

        $token = $config->parser()->parse($idtoken);
        $constraint = new \Lcobucci\JWT\Validation\Constraint\SignedWith($config->signer(), $config->signingKey());

        return $config->validator()->validate($token, $constraint);
    }
}